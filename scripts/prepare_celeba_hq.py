...
df = pd.read_csv(os.path.join(CELEBAHQ_PATH, "CelebA-HQ-to-CelebA-mapping.txt"), sep='\s+')

# get original splits
split = pd.read_csv(os.path.join(CELEBA_PATH, "list_eval_partition.txt"), sep='\s+', header=None)
...
# get attributes
attr = pd.read_csv(os.path.join(CELEBA_PATH, "list_attr_celeba.txt"), sep='\s+', skiprows=1)
